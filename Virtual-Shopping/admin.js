...
function drawList(response) {
    ...
    for (let i in response) {
        if (response.hasOwnProperty(i) && response[i].existsInSearch !== "no") {
            listContainer.innerHTML += `
      <div class="produse">
                <div class="imagine">
                        <img src="${response[i].imagine}" alt="Guitar" height="100px">
                </div>
                <div class="nume">
                ${response[i].nume}
                </div>
                ...
                <div class="pret">
                ${response[i].pret}<span>€</span>
                </div>
                <div class="stock">
                        Stoc:<span>${response[i].stoc}</span>
                </div>
                <div class="options">
                    <div id="editare">
                        <a class="btn btn-medium-detalii btn-editare  btn-radius" onclick="editDetails('${i}')" href="#" >edit</a>
                    </div>
                    
                    <div id="stergere">
                            <a class="btn btn-medium-detalii btn-stergere  btn-radius" onclick="deleteEntry('${i}')" href="#" >delete</a>
                    </div>
                </div>        
        </div>
      `
